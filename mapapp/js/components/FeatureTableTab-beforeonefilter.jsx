...
class FeatureTableTab extends React.Component {
  ...
  // _filterByText(evt) {  // this needed for textfield without autocomplete
    ...
    if (this.state.searchText !== '' && this.state.operator !== '' && this.state.searchText2 !== '') {
	    ...
	    if (filteredRows.length > 0) {
	       ...
	       //    error: true,
	    	...
	    	this.setState({
	          ...
	          errorOpen: true,
	          error: true,
	          msg: 'filter expression incomplete (1)',
	          filterActive: false
	        });
	    	filteredRows = rows;
		    this._filtered = (rows.length !== filteredRows.length);
		...
		this.setState({
	      ...
	      errorOpen: true,
		  error: true,
		  msg: 'filter expression incomplete (2)',
		  filterActive: false
		});
    	filteredRows = rows;
	    this._filtered = (rows.length !== filteredRows.length);
  ...
  _filterAllByText(evt) {
    ...
    // add all layers to FeatureStore.. otherwise added layers will not be recognized (later in render) by filter all until they were selected in layer selector
    // if filtered layer exists, delete
    var map = this.props.map;
    ...
    // filter rows by expression and push into filteredRows; plus use toggle for showing only props from specs
    var filteredRows = [];
    var queryFilter;
    if (filterBy !== '') {
      try {
      ...
      if (queryFilter !== null) {
        ...
        // hide all layers to only show filter layer
...
//aber der nimmt die ganze zeit weiterhin die 'polygons', also kann value auch nicht wirklich das entscheidende sein
  ...
  render() {
    ...
    var unique = [...new Set(cellsArray)].sort(); // originally, just using cellsArray is fine, but this eliminates dublicates in autocomplete
          ...
          <CardText style={{padding: '0px'}} expandable={false}>
            ...
            <div className='feature-table-options'>
              <div className='feature-table-selector' style={{display: this.props.layer ? 'none' : 'block'}}>
                ...
                <Checkbox label="filter all layers" onCheck={this.handleCheck.bind(this)} style={{width: 200, paddingLeft: 20}} />
              </div>
